	...
	for k in range(iter_sigmas.shape[0]):
		...
		for repeat_idx in range(num_repeat):
			...
			for iter in range(MaxIter):
				...
				exp_suff_stats1, exp_suff_stats2 = VIPS_BLR_MA.VBEstep_private(sigma, xtrain_m, ytrain_m,
				                                                               exp_nat_params_prv, S)

				if iter == 0:
					nu_old = []
					ab_old = []
				nu_new, ab_new, exp_nat_params, mean_alpha, Mu_theta = VIPS_BLR_MA.VBMstep_stochastic(rhot, nu_old,
				                                                                                      ab_old, N, a0,
				                                                                                      b0,
				                                                                                      exp_suff_stats1,
				                                                                                      exp_suff_stats2,
