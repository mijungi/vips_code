...
for S in batchmat:
	...
	for eps in epsilonmat:
		epsilon = eps
		method = 'static_results/static_private_seed=%s_J=%s_S=%s_priv=%s_epsilon=%s_compo=%s_D=%s' % (
		seednum, documentstoanalyze, batchsize, priv, epsilon, comp, D)
	...
	# method = 'static_results/static_nonprivate_seed=%s_J=%s_S=%s_priv=%s_epsilon=%s_compo=%s_D=%s' %(seednum,
# documentstoanalyze, batchsize, 0, 0, 0, D)
	# pp = np.load(method+'.npy')
